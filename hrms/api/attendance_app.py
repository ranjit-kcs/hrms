...
def create_employee_checkin(
	...
	try:
		...
		try:
			...
			if "start_time" in str(ae) or "'NoneType' object" in str(ae):
				doc.offshift = 1
...
def get_leave_approver_for_employee(employee_id):
	return frappe.db.get_value("Employee", employee_id, "leave_approver")


@frappe.whitelist()
def create_leave_application(
	employee, leave_type, from_date, to_date, half_day=0, description=None, leave_approver=None
):
	try:
...
def update_attendance_request(name, reason, from_date, to_date, explanation=None):
	try:
		...
		if doc.docstatus != 0 and getattr(doc, "status", None) != "Pending":
			frappe.throw(_("Only Draft or Pending records can be updated."))
