...
<script setup>
import { inject, markRaw, ref, onMounted } from "vue"
...
const { preload } = useCheckinCache()

onMounted(() => {
  refreshKey.value++

  // ── kick off all background preloads ──────────────────────────────────────
  // pass the user image URL so the face descriptor is ready before the user
  // ever taps Check-In / Check-Out.
  preload(user?.data?.user_image)
})
